...
async def fetch_proxy_list() -> list[str]:
    """Busca candidatos HTTP brasileiros em um provedor publico de proxies."""
    url = "https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=5000&country=BR&ssl=all&anonymity=all"
    logger.info("Buscando candidatos de proxy HTTP na API proxyscrape...")
    try:
        def _fetch():
...
async def get_free_working_proxy() -> str:
    """
    Busca e testa candidatos de proxy concorrentemente.
    Retorna o primeiro candidato validado para a rota alvo.
    """
    proxies = await fetch_proxy_list()
    if not proxies:
        raise RuntimeError("Nenhum proxy retornado pela API pública.")
        
    logger.info(f"{len(proxies)} candidatos de proxy encontrados. Iniciando validacao concorrente...")
    
    # Limita o fan-out para manter o probe sob controle operacional.
    batch_size = 20
    for i in range(0, min(60, len(proxies)), batch_size):
        ...
        while pending:
            ...
            for task in done:
                ...
                if result:
                    for t in pending:
                        t.cancel()
                    logger.info(f"Proxy validado para rota alvo: {result}")
                    return result

    raise RuntimeError("Nenhum candidato de proxy respondeu a tempo para a Betano.")
